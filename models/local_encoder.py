...
class TemporalEncoder(nn.Module):
    ...
    def forward(self,
        ...
        x = x + self.pos_embed
        out = self.transformer_encoder(src=x, mask=self.attn_mask, src_key_padding_mask=None)#[21,N,D]
        return out[-1]  # [N, D]
...
class ALEncoder2(MessagePassing):
    ...
    def __init__(self,
                 ...
                 dropout: float = 0.1,
                 **kwargs) -> None:
        super(ALEncoder2, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.embed_dim = embed_dim
        self.num_heads = num_heads

        self.lane_embed = MultipleInputEmbedding(in_channels=[embed_dim, edge_dim], out_channel=embed_dim)
        self.lin_q = nn.Linear(embed_dim, embed_dim)
        self.lin_k = nn.Linear(embed_dim, embed_dim)
        self.lin_v = nn.Linear(embed_dim, embed_dim)
        self.lin_self = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self,
        ...
        #turn_directions = turn_directions.long()
        #traffic_controls = traffic_controls.long()
        x_actor = x_actor + self._mha_block(self.norm1(x_actor), x_lane, edge_index, edge_attr, 
    ...
    def message(self,
                ...
                size_i: Optional[int]) -> torch.Tensor:
        if rotate_mat is None:
            x_j = self.lane_embed([x_j, edge_attr],None)
                                  #[self.is_intersection_embed[is_intersections_j],
                                 #  self.turn_direction_embed[turn_directions_j],
                                 #  self.traffic_control_embed[traffic_controls_j]])
        else:
            rotate_mat = rotate_mat[edge_index[1]]
            x_j = self.lane_embed([x_j,#torch.bmm(x_j.unsqueeze(-2), rotate_mat).squeeze(-2),
                                   torch.bmm(edge_attr.unsqueeze(-2), rotate_mat).squeeze(-2)],
                                   None)
                                  #[self.is_intersection_embed[is_intersections_j],
                                  # self.turn_direction_embed[turn_directions_j],
    ...
    def _mha_block(self,
        ...
        x_actor = self.out_proj(self.propagate(edge_index=edge_index, x=(x_lane, x_actor), edge_attr=edge_attr,
                                               ...
                                               rotate_mat=None, size=size))#rotate_mat, size=size))
        return self.proj_drop(x_actor)
